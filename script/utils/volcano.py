...
def load_volcano_data(csv_path, label_path):
    ...
    compartments = ["14-3-3","14-3-3","actin","autophagosome","autophagosome","centrosome","endo/lysosome","endocytic","endosome","ER",
    "ER","ER","ER","ER/Golgi","ER/Golgi","ER/Golgi","ER/Golgi","Golgi","Golgi","Golgi","HSP70 chaperone",
    "HSP90 chaperone","lysosome","lysosome","mitochondria","p-body","p-body","peroxisome","endocytic",
...
def load_volcano_data_v2(csv_path, label_path):
    '''
    v2 update: use pulldown names as column names
    '''
    volcano_df = pd.read_csv(csv_path, header=[0, 1], index_col=0)
    ...
    pval_df.loc[:,"Gene_name_canonical"] = volcano_df[('metadata', 'Gene_name_canonical')].copy()
    pval_df.set_index("Gene_name_canonical", inplace=True)
    
    enrich_df = volcano_df.xs("enrichment", axis=1, level=1, drop_level=True)
    enrich_df = enrich_df.copy()
    enrich_df.loc[:,"Gene_name_canonical"] = volcano_df[('metadata', 'Gene_name_canonical')].copy()
    enrich_df.set_index("Gene_name_canonical", inplace=True)
    
    return volcano_df, highlights, winning_pulldowns, compartments